...
import { Card, CardHeader, CardBody, Input, RadioGroup } from "@heroui/react";
import { Answer, Question } from "@/types";
import EditableAnswer from "./EditableAnswer";

type EditableQuestionProps = {
  question: Question;
  handleQuestionChange: <K extends keyof Question>(
    field: K,
    value: Question[K],
  ) => void;
  handleAnswerChange: (
    answerIndex: number,
  ) => <K extends keyof Answer>(field: K, value: Answer[K]) => void;
...
export default function EditableQuestion({
  ...
  const onlyOneCorrectAnswer =
    correctAnswer === question.answers.findLastIndex((a) => a.correct);
  ...
  return (
    <Card className="shadow-lg overflow-hidden border border-indigo-100 bg-white">
      ...
      <CardBody className="p-6 space-y-6 bg-gray-50">
        <p className="w-full text-left text-small text-default-800">
