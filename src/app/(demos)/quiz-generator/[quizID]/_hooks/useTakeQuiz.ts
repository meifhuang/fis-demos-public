...
export const useTakeQuiz = (id: string) => {
  ...
  const handleCorrectAnswerChange =
    (questionIndex: number) => (correctAnswerIndex: number) => {
      setQuiz((previous) => {
        ...
        const result = previous.withCorrectAnswer(
          questionIndex,
          correctAnswerIndex,
        );
        return result;
  ...
  const getScore = () => {
    ...
    const correctQuestions = state.quiz.questions.filter((question, index) => {
      ...
      const correctOnBase = state.baseQuiz?.questions[index].answers.find(
    ...
    return (
      Math.round((correctQuestions.length / state.quiz.questionCount) * 1000) /
      10
    );
  };
