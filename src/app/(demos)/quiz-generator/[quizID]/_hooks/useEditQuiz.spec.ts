...
describe("useEditQuiz", () => {
  ...
  test("updates correct answer to the new answer", () => {
    ...
    const newCorrectIndex = quiz.questions[0].answers.findIndex(
      (a) => !a.correct,
    );
    ...
    expect(
      answers.every(
        (answer, index) => !answer.correct || index === newCorrectIndex,
      ),
    ).toBe(true);
  ...
  test("doesn't call mutate and rejects when asked to save invalid data", async () => {
    ...
    await expect(state().saveChanges()).rejects.toBe(
