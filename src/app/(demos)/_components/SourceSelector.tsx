...
export function SourceSelector({
  ...
  const updateCustomSource = <T extends keyof SourceMaterial>(
    name: T,
    value: SourceMaterial[T],
  ) => {
    const newSource = {
      ...customSource,
      [name]: value,
  ...
  return (
    <div className="flex flex-col gap-3">
      <Select
        ...
        placeholder={
          isLoading ? "Loading source materials..." : "Select source"
        }
        labelPlacement="outside"
        startContent={isCustom ? <BookText size={18} /> : <Book size={18} />}
        selectedKeys={value ? [value] : []}
        onSelectionChange={(key) => updateSource(key.currentKey ?? "")}
      ...
      >
        <>
          ...
          <SelectItem
            ...
            data-testid="select-custom-material"
          >
            Custom
          </SelectItem>
        </>
      ...
      {isCustom ? (
        ...
        <div className="flex flex-col gap-4 p-4 rounded-lg border bg-gray-50">
          <Input
            ...
            onChange={(e) => updateCustomSource("title", e.target.value)}
            labelPlacement="outside"
            fullWidth
            required
          ...
          <Textarea
            ...
            onChange={(e) => updateCustomSource("markdown", e.target.value)}
            labelPlacement="outside"
            fullWidth
            rows={6}
            required
